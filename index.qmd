...
My choice of variables is somewhat inspired by papers like @boeckx2014effectiveness, who perform SVAR analyses of the ECB's monetary policy, however, I am instead interested in examining the forecasting ability of macroeconomic variables combined with financial for the euro area. 
...
I chose to include financial stress, which is measured by composite indicator variable for systemic stress (CISS) in the euro area, as financial conditions and markets highly affects the economic activity, inflation and unemployment. CISS summarizes the financial stress across money, bond, equity, and foreign exchange markets,
and financial intermediaries in the euro area constructed by @hollo2012ciss. When assessing the influence from financial variables in the forecasts, I remove this variable.
...
Thus, the baseline model follow the Minnesota prior as introduced by @doan1984forecasting set as:
...
The chosen extension will be analyzing the mean of $A$. 
\begin{align}
...
BVAR.extension = function(Y,X,priors,S){
  ...
  A.posterior        = array(0.0001, dim = c(K,N,sum(S))) # Cant run if I replace 0.0001 with NA
...
However, as shown by simulation (shown at the end of the report) the model extension is not working completely correctly as the model is not wholly and A posterior is possibly not calculated the correct way.
...
\end{align}
Where $h_{t}$ is the stochastic volatility process. 
...
# Bayesian VAR Stochastic Volatility with extension

Finally I combine the extension with the stochastic volatility, which can be estimated by:
...
BVAR.SV.extension = function(Y,X,priors,S){
  ...
  A.posterior        = array(0.0001, dim = c(K,N,sum(S))) # Cant run with NA replacing 0.0001
  Sigma.posterior    = array(0.0001,dim=c(N,N,sum(S))) # Cant run with NA replacing 0.0001
  sigma2.posterior    = matrix(NA, nrow(Y), sum(S)) 
...
Interestingly enough, this model show signs of robustness. However, the first row of the A posterior matrix is obviously not correct.
...
Additionally based on the argument from @sims1999error considering the usage 68$\%$ or 95$\%$ confidence bands for the forecasts, I follow their approach suggesting that 68$/%$ confidence bands can be more useful than more than the 95$\%$ confidence band.
...
Then the stochastic volatility model exhibit significantly lower uncertainty compared to the baseline model with the general forecast predicting an increase in industrial production in the next two years.
Therefore, opting to switch the volatility to stochastic volatility possibly yields more useful forecasts. 
...
## Bayesian VAR SV extension forecasting
...
# Applying BVAR function for SV extension model
posterior.draws.SV.extension.no_finance = BVAR.SV.extension(Y=Y, X=X, priors=priors, S=S)
...
for (s in 1:S){
  ...
  for (i in 1:h){
    x.T         = c(1,as.vector(t(x.Ti)))
    Y.h[i,,s]   = rmvnorm(1, mean = x.T%*%posterior.draws.SV.extension.no_finance$A.posterior[,,s], sigma=posterior.draws.SV.extension.no_finance$Sigma.posterior[,,s])
    x.Ti        = rbind(Y.h[i,,s],x.Ti[1:(p-1),])
  }
...
This research project aimed to analyze how Bayesian VAR models can be utilized to assess whether including financial variables have any predicting power for forecasting economic activity measured by industrial production but finds no results. Additionally the paper is currently lacking as the extension have to be finished. Finally the forecasts only made usage of up to 7 variables due to the late change in model extension, so the amount of variables should be increased.
